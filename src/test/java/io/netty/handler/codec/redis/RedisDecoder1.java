...
public class RedisDecoder1 extends ByteToMessageDecoder {

    private final RedisDecoder1.ToPositiveLongProcessor toPositiveLongProcessor = new RedisDecoder1.ToPositiveLongProcessor();
    ...
    private RedisDecoder1.State state = RedisDecoder1.State.DECODE_TYPE;
     ...
     */
    public RedisDecoder1(boolean decodeInlineCommands) {
     ...
     */
    public RedisDecoder1(int maxInlineMessageLength, RedisMessagePool messagePool) {
     ...
     * @param decodeInlineCommands if {@code true}, inline commands will be decoded.
     */
    public RedisDecoder1(int maxInlineMessageLength, RedisMessagePool messagePool, boolean decodeInlineCommands) {
    ...
    private void resetDecoder() {
        state = RedisDecoder1.State.DECODE_TYPE;
    ...
    private boolean decodeType(ByteBuf in) throws Exception {
        ...
        state = type.isInline() ? RedisDecoder1.State.DECODE_INLINE : RedisDecoder1.State.DECODE_LENGTH;
        return true;
    }
    ...
    private boolean decodeBulkString(ByteBuf in, List<Object> out) throws Exception {
        switch (remainingBulkLength) {
            case RedisConstants.NULL_VALUE: // $-1\r\n
                ...
                return true;
            case 0:
                state = RedisDecoder1.State.DECODE_BULK_STRING_EOL;
            ...
            default: // expectedBulkLength is always positive.
                ...
                state = RedisDecoder1.State.DECODE_BULK_STRING_CONTENT;
